...
describe('capitalizeJsxTags', () => {
  test('should capitalize JSX tags', () => {
    const input: string = '<table><tr><td>Hello</td></tr></table>';
    const output: string = '<Table><Tr><Td>Hello</Td></Tr></Table>';
    expect(capitalizeJsxTags(input)).toBe(output);
  });

  test('should not capitalize non-JSX tags', () => {
    const input: string = '<div>Hello</div>';
    const output: string = '<div>Hello</div>';
    expect(capitalizeJsxTags(input)).toBe(output);
  });
});
...
describe('copyAndRenameFiles', () => {
  ...
  const SRC_DIR: string = path.join(TEST_DIR, 'src');
  const TARGET_DIR: string = path.join(TEST_DIR, 'target');
  ...
  beforeAll(() => {
    ...
    const fileContent: string = '<table><tr><td>Hello</td></tr></table>';
  ...
  test('should copy and rename files correctly', () => {
    ...
    const targetFile: string = fs.readFileSync(path.join(TARGET_DIR, 'test.mdx'), 'utf8');
    const nestedTargetFile: string = fs.readFileSync(path.join(TARGET_DIR, 'nested', 'nested.mdx'), 'utf8');
